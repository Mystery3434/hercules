...
jQuery(document).ready(function($){
	...
	SchedulePlan.prototype.initEvents = function() {
		...
		// select the current slot and close modal window
		this.modal.on('click', '.book', function(event){
			event.preventDefault();
			// console.log($(this).parent());
			// state="selected-slot";
			// state="event-1";
		...
		this.modal.on('click', '#reschedule_booking_button', function(event){
			...
			console.log("{{lesson_to_reschedule}}");
			// console.log(current.parent().attr("id"));
			var date_text = current.parent().parent().text();
			var class_start = current.attr("data-start");
			// console.log(current.attr("data-end"));
		...
		this.modal.on('click', '.cancel', function(event){
			event.preventDefault();
			if (current.attr("data-event") == "selected-slot"){
				cost -= 80;
				// displayEvents();
				// updatePrice()
			}
			current.attr("data-event", "free-slot");
		...
		//close modal window
		this.modal.on('click', '.return', function(event){
			event.preventDefault();
	...
	SchedulePlan.prototype.openModal = function(event) {
		...
		// if(this.modal.attr('data-event')=="free-slot"){
		// 	this.modal.attr("data-event", "selected-slot");
		// 	state="selected-slot";
		// }
		// else{
		// 	state=oldState;
		...
		if (event.parent().attr('data-event')=="busy-slot"){
			this.modalBody.find('.event-info').load('busy_slot', function(data){
			//once the event content has been loaded
	...
	SchedulePlan.prototype.closeModal = function(event, selectSlot=false) {
		var self = this;
		var mq = self.mq();
		...
		// 	event.parent().attr('data-event', "event-1");
		// 	// $(this).parent().attr("data-event", "event-1");
		// }
	...
	function updatePrice(){
		...
		price.innerHTML = '<h5 class="form-control-label"> Total Cost: $ ' + cost + '</h5>'; 
	}
	function displayEvents(){
		// console.log(listItems);
		// TODO: Modify and pass the schedule variable to the next payment page. Perhaps use JSON? Need to learn to iterate through the tree.
		console.log(listItems);
		for (var i = 0; i<7; i++){
	...
	$("#book_slots").on('click', function(){
		...
		for (var i = 0; i<7; i++){
			...
			for (var j = 0 ; j < len; j++){
				if($(listItems[j]).attr('data-event') == "selected-slot"){
		...
		// console.log(listItems.attr("data-event"));
